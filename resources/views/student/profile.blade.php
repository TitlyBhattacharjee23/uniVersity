    ...
    <div class="container mt-4">
        ...
        <div class="card">
            <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('student.enrollments.download') }}" class="btn btn-light">
                    <i class="fas fa-download"></i> Download Enrollment History
                </a>
            </div>
            <div class="card-body">
                @if (session('success'))
                            ...
                            <tbody>
                                @foreach ($student->enrollments->sortByDesc('created_at') as $enrollment)
                                    <tr>
                                                ...
                                                <div class="card card-body p-2">
                                                    <ul class="list-unstyled mb-0">
                                                        @foreach ($enrollment->semester->courses as $course)
                                                            <li>
                                                                <strong>{{ $course->code }}
                                                                    ({{ $course->credit }})</strong> - {{ $course->name }}
                                                                <small
                                                                    class="text-muted">({{ $course->teacher->name }})</small>
                                                            </li>
                                                        @endforeach
                                                    </ul>
                                                </div>
                                        ...
                                        <td>
                                            <span
                                                class="badge bg-{{ $enrollment->status === 'approved' ? 'success' : ($enrollment->status === 'rejected' ? 'danger' : 'warning') }}">
                    ...
                    @foreach ($student->enrollments->where('status', 'approved')->sortByDesc('created_at') as $enrollment)
                        <div class="semester-results mb-4">
                            <h4 class="text-primary">
                                ...
                                <a href="{{ route('student.results.download', $enrollment->enrollment_id) }}"
                                        ...
                                        @foreach ($enrollment->semester->courses as $course)
                                            <tr>
                                                ...
                                                @if ($latestResult)
                                                    ...
                                                    <td>
                                                        <span
                                                            class="badge bg-{{ $latestResult->grade === 'F' ? 'danger' : ($latestResult->grade ? 'success' : 'secondary') }}">
                                                        ...
                                                        @if ($previousResults->isNotEmpty())
                                                            <div class="small text-muted">
                                                                ...
                                                                @foreach ($previousResults as $prevResult)
                                                                    <br>
                                                                    <span
                                                                        class="badge bg-{{ $prevResult->grade === 'F' ? 'danger' : 'secondary' }}">
                                                                        {{ $prevResult->grade }}
                                                                    </span>
                                                                    ({{ $prevResult->enrollment->semester->name }})
                                                                @endforeach
                                        ...
                                        <tfoot class="table-light">
                                            <tr>
                                                <td colspan="6" class="text-center text-muted">
                                                    <em>Semester GPA will be published after all course results are
                                                        available</em>
                                                </td>
                                            </tr>
                                        </tfoot>
